...
exports.CreateGeneration = function(rpcData, publicKey, extraNoncePlaceholder, reward, txMessages, recipients){
    ...
    var txInSequence = Math.pow(2, 32) - 1;
    ...
    var scriptSigPart1 = Buffer.concat([
        ...
        //Buffer.from("086b6f746f706f6f6c", 'hex'),
        util.serializeNumber(Date.now() / 1000 | 0),
        Buffer.from([extraNoncePlaceholder.length])
    ]);
    ...
    (txVersion & 0x7fffffff) == 4 ? util.packUInt32LE(0x9023e50a) : Buffer.from([]);
    ...
    var nExpiryHeight = (txVersion & 0x7fffffff) >= 3 ? util.packUInt32LE(0) : Buffer.from([]);
    var valueBalance = (txVersion & 0x7fffffff) >= 4 ? util.packInt64LE(0) : Buffer.from([]);
    var vShieldedSpend = (txVersion & 0x7fffffff) >= 4 ? Buffer.from([0]) : Buffer.from([]);
    var vShieldedOutput = (txVersion & 0x7fffffff) >= 4 ? Buffer.from([0]) : Buffer.from([]);
    var nJoinSplit = (txVersion & 0x7fffffff) >= 2 ? Buffer.from([0]) : Buffer.from([]);
